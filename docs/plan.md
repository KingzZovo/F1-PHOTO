...
- 云端 SaaS / 多机集群。
- GPU 推理优化（预留接口，不优先实现）。
- OCR 读取工单号（如果工单号由人工输入则不需，需要话后期加）。
- 商业 / 跨企业同名合并。
...
1. **接入层**：Web 后台 + Android APP，走 HTTP/JSON + multipart upload。
2. **服务层**：Rust axum，同进程内启动推理 worker pool。
3. **存储层**：PostgreSQL 16 + pgvector（元数据 + 特征向量 + 队列），本地文件系统存原图 / 缩略图 / 归档产物。
...
| 风险 | 描述 | 缓解 |
|---|---|---|
| 识别准确率不足 | 特别是工具/设备（同型号难区分） | 增量自学习 + 人工纠错 + 按需上专用模型 |
| CPU 推理吞吐 | 24G/20线程，突发上传会排队 | 后台 worker 限并发，接口立即返回 |
| 冷启动体验 | 首次上传都需手填 | 提供“快速扫脸 / 扫工具” 初始中心，一次性建库 |
| Windows 纯离线 | 依赖 onnxruntime DLL | 随包发布，预装 VC++ Redist |
| pgvector 性能 | 超过 10万 embedding 后 | 启用 ivfflat / hnsw 索引，定期 reindex |
| 人脸隐私 | 本地存储人脸照 | 完全本地，不上云；DB 启用 加密文件系统可选 |
...
- 后台识别起始迟延 < 5s（50 张背压下平均）。
- 人脸识别 TOP-1 准确率 ≥ 95%（库有 50+人、每人 5+ 质量可接受照片）。
- 工具/设备识别 TOP-1 准确率 ≥ 85%（同质量样本下）。
- 冷装验证：干净 Ubuntu 22.04 / Windows 11 纯离线机啦（1 脚本 < 10分钟部署）。
