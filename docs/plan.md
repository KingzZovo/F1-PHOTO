...
- **多账号 + 项目级 RBAC**：项目仅作访问控制；人员/工具/设备为全局主数据，跨工单/跨项目识别复用。
...
### 在范围内
- 「项目 → 工单 → 人员/工具/设备 → 照片」四层结构（项目仅作访问控制）。
- **全局主数据**：人员（`employee_no` 唯一）/ 工具（`sn` 唯一）/ 设备（`sn` 唯一），跨工单、跨项目共用同一份特征值。
- **项目级 RBAC**：每个账号对每个项目独立配置 `view / upload / delete / manage` 4 个权限位。
- **管理员全局检索**：`/api/admin/*` 跨项目检索工单 / 照片 / 识别条目。
- 上传、识别、归档、检索、打包下载。
- 特征值匹配 + 增量自学习（同身份多 Embedding 保留），全局表。
- 人脸角度分类（正 / 侧 / 背）。
- 后台可视化调参与人工纠错。
...
1. **接入层**：Web 后台 + Android APP，走 HTTP/JSON + multipart upload。工单/照片请求带 `project_id`；主数据请求不带。
2. **服务层**：Rust axum，同进程内启动推理 worker pool，包含项目权限中间件（`RequireProjectPerm`）与管理员中间件（`RequireAdmin`）。
3. **存储层**：PostgreSQL 16 + pgvector；工单/照片按 `project_id` 划分，人员/工具/设备 + 特征值库为全局表。本地文件系统存原图 / 缩略图 / 归档产物，归档路径以 `project_code` 开头。
...
### M1 地基（1.5–2.5 周）
- Rust 骨架、配置、日志、错误。
- DB 迁移，启用 pgvector，初始化 `default` 项目 + admin 加成员。
- JWT 鉴权、关闭开放注册、`admin / member` 角色。
- **项目 + 成员管理**：CRUD `projects` + `project_members`，axum extractor `RequireProjectPerm` / `RequireAdmin`。
- **全局主数据 CRUD**：人员 / 工具 / 设备（`/api/persons` 等，admin 写 / 所有账号读）。
- **项目作用域 CRUD**：工单 / 照片 / 识别条目（`/api/projects/{pid}/...`）。
- 上传 + 哈希去重（项目内）+ WebP 缩略图。
- Vue 骨架 + 登录页 + 项目切换器 + 列表页。
- admin 控制台「项目管理」+ 「人员/工具/设备主数据」页。

### M2 推理 v1（1–2 周）
- onnxruntime 接入（动态加载模型、线程池隔离）。
- 人脸检测 + embedding。
- pgvector kNN 查询（**全局**，仅按 `owner_type` 过滤）。
- 未匹配 / 增量学习决策逻辑（增量写全局表）。
...
- 识别条目列表（按项目）+ 红框预绘原图 + 人工纠正（选 owner 时全局搜索）。
- 纠正后自动写入新 embedding 到全局表。

### M5 体验层（1 周）
- 按工单 / 人员 / 工具 / 设备检索 + 打包 zip 下载。
- admin 跨项目打包 (`/api/admin/packaging`)。
...
| 冷启动体验 | 首次上传都需手填 | 「快速建档」入口；跨项目复用全局主数据 |
| 项目误用 | 工单建错项目 | admin 「迁移工单到其他项目」工具 |
| 主数据污染 | 普通账号乱填员工号 | 写入仅 admin；「自动建档」默认关 |
| Windows 纯离线 | 依赖 onnxruntime DLL | 随包发布，预装 VC++ Redist |
| pgvector 性能 | 超过 10万 embedding 后 | HNSW，定期 reindex；需要时拆 partial index |
...
- **ADR-006**（修订）：引入「项目」层，仅作**访问控制 + 组织维度**，不做数据物理隔离；admin 跨项目可达。
- **ADR-007**（反转）：人员 / 工具 / 设备 + `identity_embeddings` **全局共享**，跨项目同身份自动识别为期望行为。`employee_no` / `sn` 全局唯一。
- **ADR-008**：弃用 `operator / viewer` 全局角色；项目内 4 个布尔权限位。
- **ADR-009**：全局主数据写入仅限 admin；「上传时自动建档」默认关，全局 settings 可切换。
- **ADR-010**：`/api/admin/*` 命名空间专用于 admin 跨项目检索与操作；普通账号 403。

## 8. 验收指标

- 上传返回延迟 < 300ms（10MB 以内）。
- 后台识别起始延迟 < 5s（50 张背压下平均）。
- 人脸识别 TOP-1 准确率 ≥ 95%（全局库有 50+ 人、每人 5+ 质量可接受照片，跨项目验证）。
- 工具/设备识别 TOP-1 准确率 ≥ 85%（同质量样本下）。
- 项目权限硬指标：跨项目接口越权调用必须返 `PROJECT_FORBIDDEN` / `FORBIDDEN`，无任何越权读写。
- 全局主数据唯一性硬指标：`employee_no` / `sn` 重复插入返 `CONFLICT`。
